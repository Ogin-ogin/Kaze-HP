  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const updateResponse = await fetch('/api/page-contents/update', {
        ...
        body: JSON.stringify({ page, section: 'hero', field: 'background', value: url }),
      });
  ...
  const handleUrlSubmit = async () => {
    ...
    try {
      const updateResponse = await fetch('/api/page-contents/update', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ page, section: 'hero', field: 'background', value: urlInput }),
      });
                ...
                onClick={async () => {
                  if (confirm('背景画像を削除しますか？')) {
                    await fetch('/api/page-contents/update', {
                      method: 'POST',
                      headers: { 'Content-Type': 'application/json' },
                      body: JSON.stringify({ page, section: 'hero', field: 'background', value: '' }),
                    });
                    setBackgroundImage('');
                    alert('背景画像を削除しました');
                  }
