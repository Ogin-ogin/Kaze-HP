...
export async function getNewsFromSheets() {
  try {
    ...
    if (!auth || !process.env.SPREADSHEET_ID) {
    ...
    const response = await sheets.spreadsheets.values.get({
      ...
      range: 'News!A2:E', // Newsシート: A列=ID, B列=タイトル, C列=日付, D列=サムネイルURL, E列=内容
    });
...
export async function getConcertsFromSheets() {
  try {
    ...
    const response = await sheets.spreadsheets.values.get({
      spreadsheetId: process.env.SPREADSHEET_ID,
      range: 'Concerts!A2:G', // Concertsシート: A列=ID, B列=タイトル, C列=日付, D列=場所, E列=サムネイルURL, F列=受賞, G列=詳細
    });
...
export async function updateNewsInSheets(
  ...
  try {
    ...
    await sheets.spreadsheets.values.update({
      spreadsheetId: process.env.SPREADSHEET_ID,
      range: `News!B${rowNumber}:E${rowNumber}`,
      valueInputOption: 'RAW',
      requestBody: {
...
export async function updateConcertInSheets(
  ...
  try {
    ...
    await sheets.spreadsheets.values.update({
      spreadsheetId: process.env.SPREADSHEET_ID,
      range: `Concerts!B${rowNumber}:G${rowNumber}`,
      valueInputOption: 'RAW',
      requestBody: {
...
export async function getHistoryFromSheets() {
  try {
    ...
    const response = await sheets.spreadsheets.values.get({
      spreadsheetId: process.env.SPREADSHEET_ID,
      range: 'History!A2:C', // A列: ID, B列: 日付, C列: イベント
    });
...
export async function updateHistoryInSheets(
  ...
  try {
    ...
    await sheets.spreadsheets.values.update({
      spreadsheetId: process.env.SPREADSHEET_ID,
      range: `History!B${rowNumber}:C${rowNumber}`,
      valueInputOption: 'RAW',
      requestBody: {
...
export async function updatePageContentInSheets(
  ...
  try {
    ...
    if (rowIndex === -1) {
      ...
      await sheets.spreadsheets.values.update({
        spreadsheetId: process.env.SPREADSHEET_ID,
        range: `PageContents!D${rowNumber}`,
        valueInputOption: 'RAW',
        requestBody: {
          values: [[value]],
        },
